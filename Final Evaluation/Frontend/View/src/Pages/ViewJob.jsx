...
const ViewJob = () => {
        ...
        return (
                ...
                <
                h3 > { job.title } { getJobMode(job.remoteOffice) }
                On { job.jobType }
                At { job.company } <
                /h3> <
                /section> <
                ...
                h1 > { job.title } < /h1> {
                    ...
                    < Button onClick = {
                            () => navigate(`/job/edit/${job._id}`) } > < FaPencil / > Edit Job < /Button>} <
                        /div> { job.location ? < p > { job.location } | India < /p> : <p>Location Not Mentioned"</p > } <
                        div className = { `${styles.money_duration}` } >
                        <
                        div >
                        <
                        span > < PiMoneyFill / > Stipend < /span><span>{job.monthlySalary? `Rs ${job.monthlySalary}/month
                    ` : "Not Disclosed"}</span>
            </div>
            <div>
              <span><FaCalendar/> Duration </span><span>{job.duration ? `
                    $ { job.duration }
                    Months ` : "Not Mentioned"}</span>
            </div>
          </div>
          {
            job.aboutCompany && 
